...
#include <iostream>
#include <cstdlib>
#include "tbb/tbb_machine.h" // __TBB_Log2()
...
namespace utility{
    ...
    namespace internal {
        ...
        int step_function_exponential(const int previous,const double steps){
            assert(1<=previous); // needed for __TBB_Log2()
            const int floor_log2_previous = __TBB_Log2(previous);
            const int int_steps = static_cast<int>(steps);
            if (!( static_cast<double>(int_steps)==steps && 1<=int_steps && int_steps<=(1<<(std::min(static_cast<int>(sizeof(int)),4)*8-2)) && (int_steps&(int_steps-1))==0 ))
                throw std::invalid_argument("#steps must indicate a power of 2 in the range [1,2^30]"); // assuming 4<=sizeof(int)
            const int log2_steps = __TBB_Log2(int_steps); // exact
            const int stepsize = 1 << std::max(0,floor_log2_previous-log2_steps);
            const int result = (((previous+1)+(stepsize-1)) & ~(stepsize-1)); // ceil(previous+1) at granularity "stepsize" (which is a power of 2)
            return result;
        }
        ...
        step_function_descriptor step_function_descriptors[] = {
                ...
                step_function_descriptor('#',&step_function_exponential)
        };
        ...
        struct thread_range_step {
            ...
            int operator()(int previous)const {
                assert(0<=previous);
                const int result = (previous==0) ? 1 : step_function(previous,step_function_argument);
    ...
    // TODO: refactor thread_range_step+thread_number_range to make it less awkward for the step() "function" to also include "high", i.e., even if it's not already part of the computed sequence?
    //       possible ad-hoc solutions without such refactoring:
    //       - copy thread_number_range::last to thread_range_step (seems a bit like stretching things)
    //       - rename thread_number_range::step to m_step and introduce step() as a real function
    //       or maybe thread_range_step (only mentioned in this file) should be eliminated as a separate construct and reintegrated into thread_number_range?
    //       further analysis required (design philosophy? usage?)...
    // TODO: consistently use either first/last or low/high
    struct thread_number_range{
        int (*auto_number_of_threads)();
        int first; // 0<=first (0 is used as a special value)
        int last; // first<=last

        internal::thread_range_step step;

        thread_number_range( int (*auto_number_of_threads_)(),int low_=1, int high_=-1
                , internal::thread_range_step step_ =  internal::thread_range_step(internal::step_function_exponential,4)
        )
              ...
              ,step(step_)
        {
            if (!( 0<=first && first<=last )){
                throw std::invalid_argument("expecting 0<=low<=high");
            }
        }
        friend std::istream& operator>>(std::istream& i, thread_number_range& range){
            try{
                ...
                // TODO: why inconsistently use separate low/high and range's step?
                //       decide between strong guarantee and convenience!
    ...
    static const char* thread_number_range_desc="number of threads to use; a range of the form low[:high[:(+|*|#)step]],"
                                                " where 0<=low<=high are integers or 'auto' for the TBB default,"
                                                " and the optional step expression specifies how the next number of threads is chosen (default is #4)."
                                                " The special value 0 advances directly to 1 regardless of the step expression."
                                                " E.g., '12:16:+1' means 12,13,14,15,16."
                                                " E.g., '1:16:*2' means 1,2,4,8,16."
                                                " With '#steps' (which must indicate a power of 2 in the range [1,2^30]),"
                                                " the next power of 2 is approached in at most \"steps\" equal steps,"
                                                " e.g., '1:64:#4' means 1,2,3,4,5,6,7,8,10,12,14,16,20,24,28,32,40,48,56,64."
  ;
}
