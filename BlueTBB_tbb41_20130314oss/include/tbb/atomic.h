...
template<>
struct atomic_rep<0> {       // Specialization
	typedef int8_t word;
};
...
private:
    ...
    // TODO: is the exception for __SUNPRO_CC still needed with the reinterpret_cast?
    //       it was needed with previous union ptr_converter "for 64 bit types for 32 bit target" (why?)
    // TODO: if the exception is not needed anymore, to_bits_ptr() can probably be eliminated
    ...
    template<typename value_t>
    static                     typename atomic_rep<sizeof(value_t)>::word* to_bits_ptr(               value_t* value){
        typedef                typename atomic_rep<sizeof(value_t)>::word* result_type;
        #if __SUNPRO_CC
    ...
    template<typename value_t>
    static      const          typename atomic_rep<sizeof(value_t)>::word* to_bits_ptr(const          value_t* value){
        typedef const          typename atomic_rep<sizeof(value_t)>::word* result_type;
        #if __SUNPRO_CC
            return                  result_type (value);
        #else
            return reinterpret_cast<result_type>(value);
    ...
    template<typename value_t>
    static      const volatile typename atomic_rep<sizeof(value_t)>::word* to_bits_ptr(const volatile value_t* value){
        typedef const volatile typename atomic_rep<sizeof(value_t)>::word* result_type;
        #if __SUNPRO_CC
            return                  result_type (value);
        #else
            return reinterpret_cast<result_type>(value);
