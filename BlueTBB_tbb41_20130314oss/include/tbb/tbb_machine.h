...
/** This header provides basic platform abstraction layer by hooking up appropriate
    ...
    The macro __TBB_BIG_ENDIAN can be defined by the implementation or further down in this file.
    It is used only if __TBB_USE_GENERIC_PART_WORD_CAS is set (or in src/test/test_atomic.cpp),
    and must specify the layout of aligned 16-bit and 32-bit data anywhere within a process;
    the details of unaligned 16-bit or 32-bit data or of 64-bit data are irrelevant,
    but the layout must be the same at all relevant memory locations within the current process
    (page-specific endianness is allowed as long as the "other" endianness is not used for TBB).
    Possible settings, reflecting hardware and possibly (operating) system convention, are:
    -  1 for big-endian,
    -  0 for little-endian,
    -  not defined, for run-time detection (only to be used if one of the above),
    -  -1 to prevent undefined behavior if none of the above.
    ...
    __TBB_WORDSIZE
        This is the size of a pointer in bytes, i.e., for 32-bit executables it
        should be defined to 4 (even if they are executed on 64-bit hardware).
...
// __{BIG,LITTLE}_ENDIAN__ are interpreted:
// - to detect uses where one of them is defined as nonzero while the other is not defined or is defined as zero
//   (value 1 and not defined were observed on at least one GNU/Linux system and on Mac OS X),
// - to ignore any use like __ORDER_{BIG,LITTLE}_ENDIAN__ provided both have nonzero values like 4321 and 1234
//   (discussions and code examples exist; not yet observed).
// __BYTE_ORDER__ is interpreted in accordance with http://gcc.gnu.org/onlinedocs/cpp/Common-Predefined-Macros.html (not yet observed).
#ifdef __TBB_BIG_ENDIAN
    // use that value
#elif (__BIG_ENDIAN__ && !__LITTLE_ENDIAN__) || (defined(__BYTE_ORDER__) && __BYTE_ORDER__==__ORDER_BIG_ENDIAN__)
    #define __TBB_BIG_ENDIAN 1
#elif (__LITTLE_ENDIAN__ && !__BIG_ENDIAN__) || (defined(__BYTE_ORDER__) && __BYTE_ORDER__==__ORDER_LITTLE_ENDIAN__)
    #define __TBB_BIG_ENDIAN 0
#elif defined(__BYTE_ORDER__)
    #define __TBB_BIG_ENDIAN -1
#else
    // leave undefined, for run-time detection
#endif
...
#if __TBB_USE_GENERIC_PART_WORD_CAS || !defined(__TBB_BIG_ENDIAN) || (__TBB_BIG_ENDIAN==0) || (__TBB_BIG_ENDIAN==1)
#define __TBB_TEST_GENERIC_PART_WORD_CAS 1
...
//  - the environment must consistently use either big-endian or little-endian (same for all locations)
...
inline T __TBB_MaskedCompareAndSwap (volatile T * const ptr, const T value, const T comparand ) {
    ...
    for(atomic_backoff b;;b.pause()) {
        ...
        else continue;                                     // CAS failed but the bits of interest were left unchanged
    }
}
...
/** Fenced operations use volatile qualifier to prevent compiler from optimizing
    ...
    This assumes that the generated instructions will operate atomically,
    ...
    and typically also appropriate alignment of the data. **/
