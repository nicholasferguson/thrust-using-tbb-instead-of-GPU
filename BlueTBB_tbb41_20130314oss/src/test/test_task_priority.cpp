...
public:
    void RunTaskForest ( int id ) const {
        ...
        // TODO: replace literal 1 in the following comment (or at least make clear what PreemptionActivatorId is supposed to be)
    ...
    void operator() ( int id ) const {
        ...
        if ( this->m_opts & TestPreemption ) {
            if ( id == 1 && g_LeavesExecuted[0] == -1 ) {
                REMARK( "Warning: Low priority master finished too early\n" );
                ++g_TestFailures[m_testIndex];
            }
        }
        else {
            if ( id == 0 && g_LeavesExecuted[1] == -1 ) {
                REMARK( "Warning: Faster master takes too long\n" );
                ++g_TestFailures[m_testIndex];
            }
        }
    }
...
// TODO: stylistic review, maybe namespace instead of prefix?

tbb::task_group_context* TestSetPriority_ctx[1000];
tbb::task* TestSetPriority_tsk[1000];
tbb::atomic<int> TestSetPriority_spawning;
tbb::atomic<bool> TestSetPriority_finished;

class TestSetPriorityTask : public tbb::task {
    const int m_i;
public:
    TestSetPriorityTask(int i) : m_i(i) {}
    tbb::task* execute() {
        if (0 <= m_i && m_i <= 22) {
            for (int i = 1; i <= 2; ++i) {
                const int index = 10*m_i + i;
                TestSetPriority_ctx[index] = new tbb::task_group_context ( // TODO: leaked?
                    m_i == 0 ? tbb::task_group_context::isolated : tbb::task_group_context::bound );
                TestSetPriority_tsk[index] = new(tbb::task::allocate_root(*TestSetPriority_ctx[index])) TestSetPriorityTask(index);
                tbb::task::spawn(*TestSetPriority_tsk[index]);
            }
        }
        --TestSetPriority_spawning;
        ///REMARK("Task %i executing\n", m_i);
        while (!TestSetPriority_finished) __TBB_Yield();
        return NULL;
    }
};
...
void TestSetPriority() {
    ...
    // This test creates a forest of task_group_context instances by creating root tasks
    // and disregarding the specification's "Use method spawn_root_and_wait to execute the task."
    // by using just spawn() instead. Otherwise the test would (TODO: seem to?) require an application thread
    // per tree, and the trees would not be able to fork (they would be like stacks).
    // While that might be enough to catch the problem observed in release 4.1 update 3,
    // and while real programs would be similarly constrained, it seems better to make the test harder
    // and to anticipate any future changes that would make this situation more realistic.
    // TODO: is that sentence in the specification an instruction or just advice?
    //       if the former, add assertion; if the latter, rephrase as advice and add motivation (and adapt this comment accordingly)
    const int indices[] = { 1, 2, 11, 12, 21, 22, 111, 112, 121, 122, 211, 212, 221, 222 };
    const int indices_len = sizeof(indices)/sizeof(*indices);
    tbb::task_scheduler_init init(1+indices_len);
    TestSetPriority_spawning = indices_len;
    TestSetPriority_finished = false;
    tbb::task::spawn(*new(tbb::task::allocate_root()) TestSetPriorityTask(0));
    while (TestSetPriority_spawning) __TBB_Yield();
    TestSetPriority_ctx[11 ]->set_priority(tbb::priority_high);
    TestSetPriority_ctx[111]->set_priority(tbb::priority_normal);
    TestSetPriority_ctx[22 ]->set_priority(tbb::priority_high); // must not cause 111 to become priority_high again (as it did in 4.1 update 3)
    for (int i = 0; i < indices_len; ++i) {
        const tbb::priority_t priority =
            (indices[i] == 11 || indices[i] == 112 || indices[i] == 22 || indices[i] == 221 || indices[i] == 222)
            ? tbb::priority_high
            : tbb::priority_normal;
        REMARK("\r                    \rTask %i... ", indices[i]);
        ASSERT(TestSetPriority_ctx[indices[i]]->priority() == priority, NULL);
        REMARK("OK");
    }
    REMARK("\r                    \r");
    ...
    TestSetPriority_ctx[111]->cancel_group_execution();
    TestSetPriority_ctx[112]->cancel_group_execution();
    TestSetPriority_ctx[22 ]->cancel_group_execution();
    for (int i = 0; i < indices_len; ++i) {
        const bool cancelled = (indices[i] == 111 || indices[i] == 112 || indices[i] == 22 || indices[i] == 221 || indices[i] == 222);
        REMARK("\r                    \rTask %i... ", indices[i]);
        ASSERT(TestSetPriority_ctx[indices[i]]->is_group_execution_cancelled() == cancelled, NULL);
        REMARK("OK");
    }
    REMARK("\r                    \r");
    TestSetPriority_finished = true;
}
...
#else
    // It has been observed that TestEnqueueOrder() and TestSetPriority(),
    // even though they each have their own task_scheduler_init scope,
    // may influence their successors if the program doesn't first
    // take time for tasks and workers to finish (by sleeping):
    // - either TestEnqueueOrder() or an extra TestSetPriority() causes deadlock in TestSetPriority()
    //   (not all tasks get executed, so apparently fewer workers than requested are made available),
    // - TestSetPriority() (but apparently not TestEnqueueOrder()?) causes too many failures
    //   in TestPrioritySwitchBetweenTwoMasters() (even if there are several intervening tests).
    // TODO: explain, and if possibly a problem elsewhere make a separate test case and cure it
    //       note that there might even be two separate causes (unless they boil down to the same thing):
    //       - TestSetPriority() uses spawn() for root tasks (so there's no explicit joining)
    //       - TestEnqueueOrder() does not do that but still manages to sabotage TestSetPriority()
    // TODO: if it is only a problem here, try moving TestSetPriority() to the end
    //       (perhaps one or both delays can be avoided that way)
    //       and document for future maintenance
    TestEnqueueOrder();
    Harness::Sleep(1000); // TODO: how much is enough?
    TestSetPriority();
    Harness::Sleep(5*1000); // TODO: how much is enough?
#endif /* __TBB_TASK_PRIORITY */
