...
void run_continue_nodes( int p, tbb::flow::graph& g, tbb::flow::continue_node< OutputType >& n ) {
    for (size_t i = 0; i < N; ++i) {
        ...
        n.register_predecessor( *tbb::internal::punned_cast< tbb::flow::sender< tbb::flow::continue_msg > * >(&n) );
    }
...
void continue_nodes_with_copy( ) {
    ...
    for (int p = 1; p < 2*MaxThread; ++p) {
        ...
        for (size_t i = 0; i < N; ++i) {
            // TODO: different solution
            exe_node.register_predecessor( *tbb::internal::punned_cast< tbb::flow::sender< tbb::flow::continue_msg > * >(&exe_node) );
        }
