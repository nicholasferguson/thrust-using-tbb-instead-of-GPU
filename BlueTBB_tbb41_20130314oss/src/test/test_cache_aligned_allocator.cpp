...
#if __APPLE__ && !__TBB_DYNAMIC_LOAD_ENABLED
    // This should only occur during comparison testing, not in any normal use (there's no reason to link statically on this platform).
    REPORT( "Please ignore any non-TBB diagnostics until seeing \"TBB: Expected failure handled correctly\"...\n");
#endif
...
#if __APPLE__ && !__TBB_DYNAMIC_LOAD_ENABLED
    REPORT( "TBB: Expected failure handled correctly\n");
#endif
