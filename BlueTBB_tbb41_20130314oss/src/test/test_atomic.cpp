...
template<typename T>
void TestMaskedCAS( const char* name ) {
    using namespace masked_cas_helpers;
    REMARK("testing masked CAS<%s> (size=%d)\n",name,int(sizeof(tbb::atomic<T>)));
...
int TestMain () {
    ASSERT(__TBB_WORDSIZE==sizeof(intptr_t), NULL); // assumed by __TBB_CompareAndSwapW() etc.
#   if __TBB_ATOMIC_CTORS
...
#   if __TBB_TEST_GENERIC_PART_WORD_CAS
        {
            // Test __TBB_BIG_ENDIAN and pure endianness (assumed by simpler probe in __TBB_MaskedCompareAndSwap()).
            // Only testing aligned 32-bit variable; 16-bit variables simply assumed to be consistent.
            const uint32_t probe = 0x03020100;
            ASSERT (((uintptr_t)(&probe) & 3) == 0, NULL); // only aligned 32-bit access is relevant
            bool is_big_endian = true, is_little_endian = true;
            for (const char *pc_begin = reinterpret_cast<const char*>(&probe), *pc = pc_begin, *pc_end = pc_begin + sizeof(probe); pc != pc_end; ++pc) {
                if (*pc != pc_end-1-pc) is_big_endian = false;
                if (*pc != pc-pc_begin) is_little_endian = false;
            }
            ASSERT (!is_big_endian || !is_little_endian, NULL);
            const bool is_compile_time =
            #ifndef __TBB_BIG_ENDIAN
                false;
                ASSERT (is_big_endian || is_little_endian, "__TBB_BIG_ENDIAN should be defined as -1");
            #elif (__TBB_BIG_ENDIAN==1)
                true;
                ASSERT (is_big_endian, "__TBB_BIG_ENDIAN should not be defined as 1");
            #elif (__TBB_BIG_ENDIAN==0)
                true;
                ASSERT (is_little_endian, "__TBB_BIG_ENDIAN should not be defined as 0");
            #elif ( __TBB_BIG_ENDIAN==-1)
            ...
            #endif
            REMARK ("aligned 32-bit variables are %s (determined at %s)\n",
                    (is_big_endian ? "big-endian" : "little-endian"),
                    (is_compile_time ? "compile time" : "run time"));
        }
        ASSERT(sizeof(short)==2, NULL);
        #define M(what) TestMaskedCAS<what>(#what);
        M(unsigned short)
        M(short)
        M(unsigned char)
        M(signed char)
