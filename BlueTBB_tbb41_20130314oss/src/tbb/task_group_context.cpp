...
task_group_context::~task_group_context () {
    if ( __TBB_load_relaxed(my_kind) == binding_completed ) {
        if ( governor::is_set(my_owner) ) {
            ...
            if ( internal::as_atomic(my_kind).fetch_and_store<relaxed>(dying) == detached ) {
                my_node.my_prev->my_next = my_node.my_next;
                my_node.my_next->my_prev = my_node.my_prev;
            }
            else {
...
// TODO: verify that it was OK to make this code independent from src.*mptr_state assignment beforehand, at a bargain:
//       - special case for this==&src (also avoids strangely useless scan starting with my_parent)
//       - ancestor==&src still tested for_cancellation
//       if so, act on it by removing the external setting?
// TODO: should for_cancellation still be treated as a special case (was more relevant in a previous version of this contribution)?
// TODO: is the more efficient version relevant at all?
#if __TBB_TASK_GROUP_CONTEXT
template <typename T>
void task_group_context::propagate_task_group_state ( T task_group_context::*mptr_state, task_group_context& src, T new_state, bool for_cancellation ) {
#if 0 // minimal implementation, maximum confidence
    for ( task_group_context *ctx = this; ctx != NULL; ctx = ctx->my_parent ) {
        if ( ctx == &src ) {
            this->*mptr_state = new_state;
            break;
        }
    }
#else // potentially more efficient
    if (this->*mptr_state == new_state) {
        ...
        for ( task_group_context *ancestor = my_parent; ancestor != NULL; ancestor = ancestor->my_parent ) {
            if ( ancestor == &src || (for_cancellation && (ancestor->*mptr_state == new_state)) ) {
...
template <typename T>
void generic_scheduler::propagate_task_group_state ( T task_group_context::*mptr_state, task_group_context& src, T new_state, bool for_cancellation ) {
    ...
    while ( node != &my_context_list_head ) {
        ...
        if ( ctx.*mptr_state != new_state )
            ctx.propagate_task_group_state( mptr_state, src, new_state, for_cancellation );
        node = node->my_next;
...
template <typename T>
bool market::propagate_task_group_state ( T task_group_context::*mptr_state, task_group_context& src, T new_state, bool for_cancellation ) {
    ...
    if ( src.*mptr_state != new_state )
        ...
        return false;
    // TODO: this assignment seems to be redundant just after the conditional return above?
    ...
    for ( unsigned i = 0; i < num_workers; ++i ) {
        ...
        if ( s )
            s->propagate_task_group_state( mptr_state, src, new_state, for_cancellation );
    }
...
template <typename T>
bool arena::propagate_task_group_state ( T task_group_context::*mptr_state, task_group_context& src, T new_state, bool for_cancellation ) {
    return my_market->propagate_task_group_state( mptr_state, src, new_state, for_cancellation );
}
...
bool task_group_context::cancel_group_execution () {
    ...
    governor::local_scheduler()->my_arena->propagate_task_group_state( &task_group_context::my_cancellation_requested, *this, (uintptr_t)1, /*for_cancellation=*/true );
    return true;
}
...
void task_group_context::set_priority ( priority_t prio ) {
    ...
    if ( !s || !s->my_arena->propagate_task_group_state(&task_group_context::my_priority, *this, p, /*for_cancellation=*/false) )
        return;
