...
public:
    static unsigned default_num_threads () {
        ...
        if (DefaultNumberOfThreads) {
            return DefaultNumberOfThreads;
        }
        else if (const char* env = getenv("__TBB_DEFAULT_NUM_THREADS")) {
            // TODO: what about other uses of AvailableHwConcurrency (rml_server.cpp, test_rml_omp.cpp) and test_hw_concurrency.cpp?
            //       any other potential problems?
            return DefaultNumberOfThreads = static_cast<unsigned>(atoi(env));
        }
        else {
            return DefaultNumberOfThreads = AvailableHwConcurrency();
        }
    }
