...
void generic_scheduler::cleanup_local_context_list () {
    ...
    my_local_ctx_list_update.store<relaxed>(1);
    {
        ...
        while ( node != &my_context_list_head ) {
            ...
            if ( internal::as_atomic(ctx.my_kind).fetch_and_store<relaxed>(task_group_context::detached) == task_group_context::dying )
...
void generic_scheduler::free_scheduler() {
    ...
    if( (intptr_t) __TBB_FetchAndAddW( &my_small_task_count, -k )==k ) // cast for include/tbb/machine/mac_ppc.h, TODO: harmonise signedness
        NFS_Free( this );
}
...
task& generic_scheduler::allocate_task( size_t number_of_bytes,
    ...
    if( number_of_bytes<=quick_task_size ) {
        ...
        if( (t = my_free_list) ) {
            ...
            t->prefix().next = 0;
            ++my_small_task_count; // TODO: really OK to do this non-atomically?
        }
