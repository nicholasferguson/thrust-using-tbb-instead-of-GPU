...
struct ReplaceIf
{
  ...
  ReplaceIf(const Range1& X, const Range2& Y, Predicate pred, const T& new_value)
    : A(X.begin(), X.end()), A_copy(X.begin(), X.end()),
      B(Y.begin(), Y.end()),
...
struct ReplaceCopyIf
{
  ...
  ReplaceCopyIf(const Range1& X, const Range2& Y, const Range3& Z, Predicate pred, const T& new_value)
    : A(X.begin(), X.end()), A_copy(X.begin(), X.end()),
      B(Y.begin(), Y.end()),
      C(Z.begin(), Z.end()),
      pred(pred), new_value(new_value)
